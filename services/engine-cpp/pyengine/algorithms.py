...
def query_bi_dijkstra(source_edge: int, target_edge: int, data: ShortestPathData) -> Tuple[float, List[int], bool]:
    return bidirectional_dijkstra([source_edge], [target_edge], data, inside_filter=False)
...
def query_classic(source_edge: int, target_edge: int, data: ShortestPathData) -> Tuple[float, List[int], bool]:
    return bidirectional_dijkstra([source_edge], [target_edge], data, inside_filter=True)
...
def query_m2m(source_edges: List[int], target_edges: List[int], data: ShortestPathData) -> Tuple[float, List[int], bool]:
    return bidirectional_dijkstra(source_edges, target_edges, data, inside_filter=True)

# =============================================================================
# Algorithm 7: Alternative Path (penalty method)
# =============================================================================
def query_alternative(
    ...
    """Find alternative path by penalizing shortest path edges in hierarchical search."""
    if shortest_path_expanded is None:
        _, sp, success = query_classic(source_edge, target_edge, data)
    ...
    return bidirectional_dijkstra(
        [source_edge], [target_edge], data, 
        inside_filter=True, penalty_set=penalty_set, penalty_factor=penalty_factor
    )
    ...
    while pq_fwd and pq_bwd:
        ...
        if pq_fwd[0][0] <= pq_bwd[0][0]:
            ...
            for sc in data.fwd_adj.get(u, []):
                ...
                cost = sc.cost
                if penalty_set and (sc.to_edge in penalty_set or (sc.via_edge != 0 and sc.via_edge in penalty_set)): cost *= penalty_factor
                nd = d + cost
                if nd < dist_fwd.get(sc.to_edge, INF):
                    dist_fwd[sc.to_edge] = nd
                    parent_fwd[sc.to_edge] = u
            ...
            for sc in data.bwd_adj.get(u, []):
                ...
                cost = sc.cost
                if penalty_set and (sc.from_edge in penalty_set or (sc.via_edge != 0 and sc.via_edge in penalty_set)): cost *= penalty_factor
                nd = d + cost
                if nd < dist_bwd.get(sc.from_edge, INF):
                    dist_bwd[sc.from_edge] = nd
                    parent_bwd[sc.from_edge] = u
