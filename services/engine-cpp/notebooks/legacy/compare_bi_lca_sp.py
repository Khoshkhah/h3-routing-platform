"""
Compare bi_lca algorithm against Dijkstra.

Usage:
    python compare_bi_lca.py <db_path> [--samples N]
"""

import sys
import time
import random
from routing_algorithms_sp import dijkstra_sp, bi_lca_sp, load_adjacency
import duckdb


def compare(db_path: str, n_samples: int = 100):
    """Compare bi_lca to dijkstra on random samples."""
    ...
    for i in range(n_samples):
        ...
        # bi_lca
        t0 = time.time()
        bi_cost, bi_path, bi_ok = bi_lca_sp(con, src, tgt, fwd_adj, bwd_adj)
    ...
    print(f"  bi_lca avg time: {1000*bi_time/n_samples:.2f} ms")
    ...
    if mismatches:
        ...
        for m in mismatches[:10]:
            ...
            print(f"    bi_lca: {m['bi_cost']}, path={m['bi_path_len']} edges")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python compare_bi_lca.py <db_path> [--samples N]")
        sys.exit(1)
